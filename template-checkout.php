 ...
 * Description: Fully dynamic, production-ready checkout and payment authorization engine for Elite Vault Grading.
...
// -------------------------------------------------------------------------
// 2. PAYMENT PROCESSING & WORKFLOW ADVANCEMENT
// -------------------------------------------------------------------------
if ( 'POST' === $_SERVER['REQUEST_METHOD'] && isset( $_POST['evg_checkout_nonce'] ) ) {
    if ( wp_verify_nonce( sanitize_key( $_POST['evg_checkout_nonce'] ), 'evg_process_checkout_action' ) ) {
        ...
        $gateway      = isset( $_POST['payment_gateway'] ) ? sanitize_text_field( wp_unslash( $_POST['payment_gateway'] ) ) : 'card';
        ...
        if ( ! $terms_agreed ) {
            ...
            if ( ! $submission ) {
                ...
                if ( class_exists( 'Elite_Vault_Grading_System' ) && method_exists( 'Elite_Vault_Grading_System', 'log_activity' ) ) {
                    Elite_Vault_Grading_System::log_activity( "Payment Authorized via {$gateway} for Order #{$submission->order_number}. Stage set to: Cards Awaiting Arrival." );
                }
            ...
            if ( ! $marketplace_item || $marketplace_item->stock_quantity < 1 ) {
                ...
                if ( class_exists( 'Elite_Vault_Grading_System' ) && method_exists( 'Elite_Vault_Grading_System', 'log_activity' ) ) {
                    Elite_Vault_Grading_System::log_activity( "Marketplace item #{$marketplace_item->id} ({$marketplace_item->display_title}) purchased via {$gateway}." );
                }
...
<style>
  ...
  .evg-payment-cell:hover {
    background: #18181c;
    border-color: var(--evg-border-gold-faint);
  }
  .evg-payment-cell:has(input:checked) {
    background: rgba(212, 175, 55, 0.08);
    border-color: var(--evg-gold-primary);
  }

  .evg-radio, .evg-checkbox {
    appearance: none;
  ...
  .evg-radio:checked { border-color: var(--evg-gold-primary); }
  .evg-radio:checked::before { transform: scale(1); }
...
<main class="evg-master-wrapper">
    <div class="evg-container">
        ...
        <header style="text-align: center; margin-bottom: 45px; padding-bottom: 25px; border-bottom: 1px solid var(--evg-border-hairline);">
            <span class="evg-label-micro" style="margin-bottom: 10px;"><?php esc_html_e( 'Stage 03 // Finalization', 'evg-platform' ); ?></span>
            <h1 class="evg-title-xl"><?php esc_html_e( 'Secure Order', 'evg-platform' ); ?> <span class="evg-text-metallic"><?php esc_html_e( 'Authorization', 'evg-platform' ); ?></span></h1>
            <p style="color: var(--evg-text-ash); max-width: 680px; margin: 0 auto; font-size: 0.95rem; line-height: 1.6;">
                <?php esc_html_e( 'Review your item manifest, confirm your UK return logistics coordinates, and finalize authorization via our secure 256-bit encrypted gateway.', 'evg-platform' ); ?>
            </p>
        </header>
        ...
        <?php if ( 'submission' === $checkout_type && 'yes' !== $accept_submissions ) : ?>
            <div style="background: rgba(255, 69, 58, 0.08); border: 1px solid rgba(255, 69, 58, 0.3); border-radius: 6px; padding: 20px; text-align: center; margin-bottom: 30px;">
                ...
                <p style="color: var(--evg-text-ash); font-size: 0.85rem; margin: 0;">
                    <?php esc_html_e( 'We have reached maximum vault capacity for the current drop. Existing submissions can complete payment below.', 'evg-platform' ); ?>
                </p>
            </div>
        <?php endif; ?>
        ...
        <?php if ( ! empty( $checkout_error ) ) : ?>
            <div style="background: rgba(255, 69, 58, 0.08); border: 1px solid rgba(255, 69, 58, 0.3); border-radius: 4px; padding: 16px; margin-bottom: 30px;">
                ...
                <p style="color: #e5e5ea; font-size: 0.85rem; margin: 0;"><?php echo esc_html( $checkout_error ); ?></p>
            </div>
        <?php endif; ?>
        ...
        <form action="<?php echo esc_url( get_permalink() ); ?>" method="post">
            ...
            <div class="evg-checkout-grid">
                ...
                <div>
                    <div class="evg-module" style="padding: 35px 30px;">
                        ...
                        <div style="margin-bottom: 30px;">
                            ...
                            <div style="background: var(--evg-obsidian-base); border: 1px solid var(--evg-border-hairline); border-radius: 6px; padding: 18px; font-size: 0.85rem; line-height: 1.5;">
                                <p style="color: #ffffff; font-weight: 700; margin: 0 0 4px 0;"><?php echo esc_html( $customer_name ); ?></p>
                                <p style="color: var(--evg-text-ash); margin: 0 0 2px 0;"><?php echo esc_html( trim( $house_number . ' ' . $street_address ) ? trim( $house_number . ' ' . $street_address ) : __( 'Street address unverified', 'evg-platform' ) ); ?></p>
                                <p style="color: var(--evg-text-ash); margin: 0 0 2px 0;"><?php echo esc_html( trim( $town_city . ', ' . $county, ', ' ) ); ?></p>
                                <p style="color: var(--evg-gold-primary); font-weight: 700; font-family: monospace; margin: 0;"><?php echo esc_html( $postcode ? $postcode . ', United Kingdom' : __( 'UK Postal Code Required', 'evg-platform' ) ); ?></p>
                            </div>
                        </div>
                ...
                <!-- RIGHT: FINANCIAL LEDGER & CRYPTOGRAPHIC GATEWAY -->
                <div style="display: flex; flex-direction: column; gap: 24px;">
                    ...
                    <div class="evg-module" style="padding: 35px 30px;">
                        <h2 style="color: #ffffff; font-size: 1.2rem; font-weight: 700; margin: 0 0 20px 0; border-bottom: 1px solid var(--evg-border-hairline); padding-bottom: 12px;">
                            <?php esc_html_e( 'Payment Gateway', 'evg-platform' ); ?>
                        </h2>

                        <!-- Gateway Selector -->
                        <div style="display: flex; flex-direction: column; gap: 10px; margin-bottom: 20px;">
                            <label class="evg-payment-cell">
                                <input class="evg-radio" type="radio" name="payment_gateway" value="card" checked>
                                <div>
                                    <span style="display: block; color: #ffffff; font-size: 0.9rem; font-weight: 600; margin-bottom: 2px;"><?php esc_html_e( 'Debit / Credit Card', 'evg-platform' ); ?></span>
                                    <span style="color: var(--evg-text-ash); font-size: 0.75rem;"><?php esc_html_e( 'Visa, Mastercard, Maestro, American Express', 'evg-platform' ); ?></span>
                                </div>
                            </label>

                            <label class="evg-payment-cell">
                                <input class="evg-radio" type="radio" name="payment_gateway" value="apple_pay">
                                <div>
                                    <span style="display: block; color: #ffffff; font-size: 0.9rem; font-weight: 600; margin-bottom: 2px;"><?php esc_html_e( 'Apple Pay / Google Pay', 'evg-platform' ); ?></span>
                                    <span style="color: var(--evg-text-ash); font-size: 0.75rem;"><?php esc_html_e( 'Fast 1-touch biometric checkout', 'evg-platform' ); ?></span>
                                </div>
                            </label>
                        </div>
                        ...
                        <div style="background: var(--evg-obsidian-base); border: 1px solid var(--evg-border-hairline); border-radius: 6px; padding: 20px; margin-bottom: 25px;">
                            <div style="margin-bottom: 15px;">
                                <label class="evg-label-micro" style="margin-bottom: 6px;"><?php esc_html_e( 'Card Number', 'evg-platform' ); ?></label>
                                <input type="text" class="evg-form-control" placeholder="•••• •••• •••• ••••" autocomplete="cc-number">
                            </div>
                            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 12px;">
                                <div>
                                    <label class="evg-label-micro" style="margin-bottom: 6px;"><?php esc_html_e( 'Expiry Date', 'evg-platform' ); ?></label>
                                    <input type="text" class="evg-form-control" placeholder="MM / YY" autocomplete="cc-exp">
                                </div>
                                <div>
                                    <label class="evg-label-micro" style="margin-bottom: 6px;"><?php esc_html_e( 'Security Code', 'evg-platform' ); ?></label>
                                    <input type="password" class="evg-form-control" placeholder="CVC" maxlength="4" autocomplete="cc-csc">
                                </div>
                            </div>
                        </div>
                        ...
                        <button type="submit" class="btn-evg-executive" style="margin-bottom: 20px;">
                            <?php printf( esc_html__( 'Authorize £%.2f', 'evg-platform' ), $total_payable ); ?>
                        ...
                        <div style="display: flex; align-items: center; justify-content: center; gap: 8px; color: var(--evg-text-ash); font-size: 0.7rem; font-family: monospace;">
                            ...
                            <span><?php esc_html_e( '256-BIT TLS ENCRYPTED VAULT SESSION', 'evg-platform' ); ?></span>
                        </div>
                    </div>
        ...
        <div style="margin-top: 50px; padding-top: 40px; border-top: 1px solid var(--evg-border-hairline);">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; text-align: center;">
                <div style="background: var(--evg-obsidian-panel); border: 1px solid var(--evg-border-hairline); border-radius: 6px; padding: 25px 20px;">
                    ...
                    <p style="color: #ffffff; font-size: 0.85rem; line-height: 1.5; margin: 0;"><?php esc_html_e( 'Payment is verified via secure 256-bit encrypted gateway handshake.', 'evg-platform' ); ?></p>
                </div>
                <div style="background: var(--evg-obsidian-panel); border: 1px solid var(--evg-border-hairline); border-radius: 6px; padding: 25px 20px;">
